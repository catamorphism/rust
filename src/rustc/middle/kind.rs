...
fn check_fn(fk: visit::fn_kind, decl: fn_decl, body: blk, sp: span,
    ...
    do with_appropriate_checker(cx, fn_id) |chk| {
        ...
        for vec::each(*freevars::get_freevars(cx.tcx, fn_id)) |fv| {
            ...
            let is_move = false; // FIXME (#2633): finish removing last-use
...
fn check_expr(e: @expr, cx: ctx, v: visit::vt<ctx>) {
    ...
    match e.node {
      ...
      expr_copy(expr) => check_copy_ex(cx, expr, false,
                            Some(("Explicit copy requires a copyable \
                                   argument", ""))),
      ...
      expr_rec(fields, def) | expr_struct(_, fields, def) => {
        ...
        match def {
          ...
          // Note: you can't currently extend a struct with ".." since
          // the base expr may not be copyable.
          // FIXME #3481
              ty::ty_rec(f) => f,
