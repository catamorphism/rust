        ...
        foreach lib in libfiles.iter() {
            ...
            match has_library(&lib) {
                Some(basename) => {
                    ...
                    let rel_p = path::get_relative_to(&p.push("lib/"), &lib);
...
pub fn has_library(p: &Path) -> Option<~str> {
    ...
    for files.iter().advance |q| {
        let as_path = Path(*q);
        if as_path.filetype() == Some(os::consts::DLL_SUFFIX.to_owned()) {
            // if filetype is Some(..), filestem must also be... right?
            let stuff : ~str = as_path.filestem().get();
