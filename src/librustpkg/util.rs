...
pub fn compile_input(ctxt: &BuildCtx,
    ...
    debug!("compile_input's sysroot = %?", ctxt.cx.sysroot_opt_str());
...
// NOTE should probably put the Exec in one of the contexts
pub fn compile_crate(ctxt: &BuildCtx, exec: &mut workcache::Exec, pkg_id: &PkgId,
    ...
    do c.each_view_item |vi| {
        ...
        match vi.node {
            ...
            ast::view_item_extern_mod(lib_ident, path_opt, _, _) => {
                debug!("*** sysroot: %s", my_ctxt.sysroot_opt().to_str());
                let lib_name = match path_opt { // ???
                    Some(p) => p, None => sess.str_of(lib_ident) };
                match find_library_in_search_path(Some(@my_ctxt.sysroot_opt()), lib_name) {
