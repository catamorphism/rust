...
fn test_extern_mod() {
    ...
    let env = Some([(~"RUST_PATH", lib_depend_dir.to_str())]);
    let rustc = rustpkg_exec().with_filename("rustc");
    let lib_dir = rustpkg_exec().pop().pop().push("lib");
    debug!("RUST_PATH=%s %s %s -L %s -o %s", lib_depend_dir.to_str(), rustc.to_str(),
            lib_dir.to_str(), main_file.to_str(), exec_file.to_str());

    let mut prog = run::Process::new(rustc.to_str(), [main_file.to_str(), ~"-L", lib_dir.to_str(),
                                               ~"-o", exec_file.to_str()],
                                     run::ProcessOptions {
        env: env.map(|v| v.slice(0, v.len())),
        dir: Some(&dir),
