...
fn correct_package_name_with_rust_path_hack() {
    ...
    - there's an error (use error code?)
    the wrong output would be that there's a file with `bar`'s name in the
    output
    also a test for the situation where everything is set up correctly,
